...
function PurchasesTab() {
  ...
  const downloadLink = trpc.downloads.getLink.useQuery(
    { orderNumber: "", itemId: 0 },
    { enabled: false }
  );

  const handleDownload = async (orderNumber: string, itemId: number) => {
    toast.info("กำลังสร้าง Download Link...");
    // Navigate to download page
    window.open(`/dashboard/download/${orderNumber}/${itemId}`, "_blank");
  };
        ...
        <div key={i} className="cyber-card p-5">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button
                onClick={() => handleDownload("", p.item.id)}
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-[rgba(204,255,0,0.1)] border border-[rgba(204,255,0,0.2)] text-[#ccff00] text-xs font-medium hover:bg-[rgba(204,255,0,0.2)] transition-all"
              >
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="px-4 py-3 rounded-xl bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.08)]">
              <span className={`text-xs font-bold px-2 py-0.5 rounded ${user?.role === "admin" ? "bg-[rgba(255,0,179,0.15)] text-[#ff00b3]" : "bg-[rgba(204,255,0,0.1)] text-[#ccff00]"}`}>
          ...
          <div>
            ...
            <div className="px-4 py-3 rounded-xl bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.08)] text-white/60 text-sm">
              {user?.createdAt ? new Date(user.createdAt).toLocaleDateString("th-TH", { year: "numeric", month: "long", day: "numeric" }) : "-"}
            </div>
          </div>
        </div>
        <p className="text-white/30 text-xs mt-4">
          ข้อมูลบัญชีจัดการผ่าน Manus OAuth ไม่สามารถแก้ไขได้โดยตรง
        </p>
      </div>
    </div>
  );
...
export default function Dashboard() {
  ...
  const activeTab = location.includes("/orders") ? "orders" : location.includes("/settings") ? "settings" : "purchases";
    ...
    <Layout>
      <div className="container py-8">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-black text-white">สวัสดี, {user?.name?.split(" ")[0] ?? "นักเทรด"}!</h1>
        ...
        <div className="flex gap-1 mb-6 border-b border-[rgba(255,255,255,0.06)]">
          {tabs.map((tab) => (
            <Link
              key={tab.id}
              href={tab.href}
              className={`flex items-center gap-2 px-5 py-3 text-sm font-medium transition-all border-b-2 -mb-px ${activeTab === tab.id ? "text-[#ccff00] border-[#ccff00]" : "text-white/50 border-transparent hover:text-white"}`}
            >
